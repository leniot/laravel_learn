        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="box box-widget">
                    ...
                    <form class="form-horizontal" pjax-container action="{{ admin_base_path('auth/roles').'/'.$role->id }}" method="post">
                        @csrf
                        {{ method_field('PATCH') }}
                        <div class="box-body">
                            <div class="fields-group">
                                <div class="form-group {{ $errors->has('identifier') ? ' has-error' : '' }}">
                                    ...
                                    <div class="col-sm-8">
                                        <input type="text" class="form-control" id="identifier" value="{{ $role->identifier }}"
                                               name="identifier" placeholder="输入角色标识">
                                    </div>
                                ...
                                <div class="form-group {{ $errors->has('name') ? ' has-error' : '' }}">
                                    <label for="name" class="col-sm-3 control-label">角 色 名 称：</label>

                                    <div class="col-sm-8">
                                        <input type="text" class="form-control" id="name" value="{{ $role->name }}"
                                               name="name" placeholder="输入角色名称">
                                    </div>
                                    @if ($errors->has('name'))
                                        <div class="col-sm-offset-3 col-sm-8">
                                            <span class="invalid-feedback">
                                    ...
                                    <div class="col-sm-8">
                                        <select multiple="multiple" size="10" name="policies[]">
                                            @foreach ($policyList as $policy)
                                                <option @foreach($role->policies as $r_policies)
                                                            @if($policy->id == $r_policies->id)
                                                            {{ 'selected' }}
                                                            @endif
                                                        @endforeach
                                                        value="{{ $policy->id }}">
                                ...
                                <div class="form-group {{ $errors->has('desc') ? ' has-error' : '' }}">
                                    ...
                                    <div class="col-sm-8">
                                        <input type="text" class="form-control" value="{{ $role->desc }}"
